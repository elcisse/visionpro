...
- ~~**Phase 5-6** : tests automatisés, durcissement/revue de code.~~ Fait — 26 tests (`tests/Feature/*`, PHPUnit, SQLite en mémoire) couvrant auth, permissions (bypass Super Admin, rôle lecture seule bloqué), `EnginStatutService`, calcul de facture, synchro paiement/facture, numéro de contrat, CRUD Engins. Audit sécurité/qualité effectué (composants Livewire, contrôleurs, modèles) : 4 failles corrigées (IDOR sur `deletePhoto`, absence de rate limiting sur le login, perte de fichier PDF si suppression de contrat échoue, upload SVG du logo).
- **Phase 7-9** (prochaine) : préparation déploiement (PHP ≥ 8.4 requis chez l'hébergeur, mot de passe admin `admin123` à changer), déploiement, recette utilisateur, suivi post-lancement.
